...
func (wc *WsClientStrap) Start() error {
	...
	wsCh := httpx.NewWsChannel(wc, conn, wsClientConf, handle, nil, false)
	err = wsCh.Start()
	if err == nil {
		wc.Channel = wsCh
		wc.Closed = false
...
func (kc *KcpClientStrap) Start() error {
	...
	kcpCh := kcpx.NewKcpChannel(kc, conn, kcpClientConf, chHandle, false)
	err = kcpCh.Start()
	if err == nil {
		kc.Channel = kcpCh
		kc.Closed = false
...
func (kc *Kws00ClientStrap) Start() error {
	...
	kwsCh := kcpx.NewKws00Channel(kc, conn, clientConf, chHandle, kc.params, false)
	err = kwsCh.Start()
	if err != nil {
		return err
	}
...
func (uc *UdpClientStrap) Start() error {
	...
	udpCh := udpx.NewUdpChannel(uc, conn, clientConf, chHandle, false)
	err = udpCh.Start()
	if err == nil {
		uc.Channel = udpCh
		uc.Closed = false
...
func (uc *TcpClientStrap) Start() error {
	...
	udpCh := tcpx.NewTcpChannel(uc, conn, clientConf, chHandle, false)
	err = udpCh.Start()
	if err == nil {
		uc.Channel = udpCh
		uc.Closed = false
